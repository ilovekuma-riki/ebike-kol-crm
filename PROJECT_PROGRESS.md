...
- Partner 工作台：新建 Partner、编辑主档、新建合作、Partner 长期备注、关联合作的跟进记录、下一行动和下次跟进时间。
- 任务工作台：新建任务、关联 Partner/合作、负责人/截止时间/优先级，以及待处理、进行中、等待对方、已完成、已取消状态更新。

## 上线前必须完成

1. 执行 `20260813120000_partner_workbench` Prisma migration 后再部署本版应用。
2. 分别配置 EU、US、CA、BR 四站 Shopify 域名与 Admin API token，再逐站执行首次同步。
3. 设置高强度 `N8N_WEBHOOK_SECRET`，并验证所有生产 API 的角色权限。
